...
async function fetchAccounts() {
  loading.value = true
  try {
    const res = await axios.get(`/api/admin/users/${props.userId}/accounts`, {
...
async function handleAdd() {
  ...
  try {
    const res = await axios.post(`/api/admin/users/${props.userId}/accounts`,
      { code: addCode.value, name: addName.value, platform: addPlatform.value },
      { headers: { 'x-admin-token': adminToken.value } },
    )
    if (res.data.ok) {
      toast.success('添加成功')
...
async function handleDelete(acc: QQAccount) {
  ...
  try {
    await axios.delete(`/api/admin/users/${props.userId}/accounts/${acc.id}`, {
      headers: { 'x-admin-token': adminToken.value },
    })
    toast.success('已删除')
    fetchAccounts()
...
watch(() => props.show, (v) => { if (v) fetchAccounts() })
</script>
  ...
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" @click.self="emit('close')">
    <div class="w-full max-w-2xl max-h-[80vh] rounded-xl bg-white shadow-xl dark:bg-gray-800 flex flex-col">
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-4">
        ...
        <div v-for="acc in accounts" :key="acc.id" class="flex items-center justify-between rounded-lg border border-gray-100 p-3 dark:border-gray-700">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="text-xs text-gray-400">{{ acc.platform === 'wx' ? '微信' : 'QQ' }} · Code: {{ acc.code?.substring(0, 8) }}...</div>
            </div>
          </div>
